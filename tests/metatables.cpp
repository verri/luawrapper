...
TEST(Metatables, WritingMetatable)
{
    ...
    context.writeFunction(
        "foo", LuaContext::Metatable, "__call",
        [&](const std::vector<std::pair<int, int>>&) { called = true; });
    EXPECT_EQ("table",
              context.executeCode<std::string>("return type(getmetatable(foo))"));
...
TEST(Metatables, WritingMetatableObjects)
{
    ...
    context.writeFunction("foo", LuaContext::Metatable, "__call",
                          [](Foo& foo) { foo.value++; });
    context.writeFunction("foo", LuaContext::Metatable, "__index",
                          [](Foo& foo, std::string index) -> int {
                              foo.value += index.length();
                              return 12;
                          });
...
TEST(Metatables, customEq)
{
    ...
    {
        int value = 0;
        bool operator==(const Foo& rhs) { return rhs.value == this->value; };
    };
    struct Bar
    {
        int value = 0;
...
TEST(Metatables, customToString)
{
    ...
    {
        int value = 0;
        const std::string toString() { return "foo bar"; };
    };
    struct Bar
    {
        int value = 0;
    ...
    EXPECT_EQ("userdata 0x",
              context.executeCode<std::string>("return tostring(bar)").substr(0, 11));
}
