...
TEST(BasicReadWrite, StringWithNUL)
{
    LuaContext context;

    context.executeCode("a = 'hello'..string.char(0)..'world'");
    EXPECT_EQ(std::string("hello\0world", 11), context.readVariable<std::string>("a"));

    context.writeVariable("b", std::string("hello\0world", 11));
    EXPECT_EQ(std::string("hello\0world", 11),
              context.executeCode<std::string>("return b"));
}
...
TEST(BasicReadWrite, TypeError)
{
    ...
    EXPECT_THROW(context.readVariable<std::function<void()>>("a"),
                 LuaContext::WrongTypeException);
    EXPECT_THROW(context.readVariable<Foo>("a"), LuaContext::WrongTypeException);
    EXPECT_THROW(context.readVariable<std::vector<int>>("a"),
                 LuaContext::WrongTypeException);
}
