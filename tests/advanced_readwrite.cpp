...
TEST(AdvancedReadWrite, VariantError)
{
    ...
    context.writeVariable("a", "test");
    EXPECT_THROW(
        (context.readVariable<boost::variant<bool, int, std::vector<double>>>("a")),
        LuaContext::WrongTypeException);
}
...
TEST(AdvancedReadWrite, ReadingOptional)
{
    ...
    EXPECT_THROW(context.readVariable<boost::optional<int>>("b"),
                 LuaContext::WrongTypeException);
...
TEST(AdvancedReadWrite, VectorOfPairs)
{
    ...
    context.writeVariable(
        "a",
        std::vector<std::pair<int, std::string>>{ { 1, "hello" }, { -23, "world" } });
...
TEST(AdvancedReadWrite, VectorOfPairsOfSharedPtr)
{
    ...
    context.writeVariable("a",
                          std::vector<std::pair<int, std::shared_ptr<std::string>>>{
...

    {
        const auto val =
            context
                .readVariable<std::vector<std::pair<int, std::shared_ptr<std::string>>>>(
                    "a");
        EXPECT_TRUE(val[0].first == 1 || val[0].first == -23);
        EXPECT_TRUE(val[1].first == 1 || val[1].first == -23);
...
TEST(AdvancedReadWrite, Maps)
{
    ...
    context.writeVariable("a",
                          std::map<int, std::string>{ { 1, "hello" }, { -23, "world" } });
...
TEST(AdvancedReadWrite, MapOfSharedPtr)
{
    ...
    context.writeVariable("a",
                          std::map<int, std::shared_ptr<std::string>>{
                              { 1, std::make_shared<std::string>("hello") },
                              { -23, std::make_shared<std::string>("world") } });
    ...
    {
        const auto val =
            context.readVariable<std::map<int, std::shared_ptr<std::string>>>("a");
...
TEST(AdvancedReadWrite, UnorderedMaps)
{
    ...
    context.writeVariable(
        "a", std::unordered_map<int, std::string>{ { 1, "hello" }, { -23, "world" } });
...
TEST(AdvancedReadWrite, UnorderedMapOfSharedPtr)
{
    ...
    context.writeVariable("a",
                          std::unordered_map<int, std::shared_ptr<std::string>>{
                              { 1, std::make_shared<std::string>("hello") },
                              { -23, std::make_shared<std::string>("world") } });
    ...
    {
        const auto val =
            context.readVariable<std::unordered_map<int, std::shared_ptr<std::string>>>(
                "a");
        EXPECT_TRUE(val.at(1) != nullptr);
        if (val.at(1) != nullptr) {
            EXPECT_EQ("hello", *val.at(1));
...
TEST(AdvancedReadWrite, AdvancedExample)
{
    ...
    context.writeVariable(
        "a",
        std::vector<
            std::pair<boost::variant<int, std::string>, boost::variant<bool, float>>>{
            { "test", true },
            { 2, 6.4f },
            { "hello", 1.f },
            { "world", -7.6f },
