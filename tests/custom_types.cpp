...
TEST(CustomTypes, MemberFunctionsReturnedObjects)
{
    ...
    context.writeFunction(
        "Object", "newSharedPtr", []() { return std::make_shared<Object>(); });
...
TEST(CustomTypes, CustomMemberFunctions)
{
    ...
    LuaContext context;
    context.registerFunction<void (Object::*)()>("increment",
                                                 [](Object& obj) { ++obj.value; });
    context.registerFunction<Object, int(int)>("add", [](Object& obj, int x) {
        obj.value += x;
        return obj.value;
    });
...
TEST(CustomTypes, CustomMembers)
{
    ...
    context.registerMember<int(Object::*)>(
        "value", [](const Object& obj) { return 2; }, [](Object& obj, int val) {});
...
TEST(CustomTypes, CopiesCheckReadWrite)
{
    ...
    struct Foo
    {
        Foo(int* copiesCount, int* movesCount)
           : copiesCount(copiesCount), movesCount(movesCount)
        {}
        Foo(const Foo& f) : copiesCount(f.copiesCount), movesCount(f.movesCount)
        {
            ++*copiesCount;
        }
        Foo(Foo&& f) : copiesCount(f.copiesCount), movesCount(f.movesCount)
        {
            ++*movesCount;
        }
